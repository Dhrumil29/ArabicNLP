...
app_id = "1454520107898316"
app_secret = "5d62384aa921b96560ea45386a43ce95" # DO NOT SHARE WITH ANYONE!
file_id = "Ghassan.Aboud.Page"
...
def scrapeFacebookPageFeedComments(page_id, access_token):
    initial_data = ['comment_id', 'status_id', 'parent_id', 'comment_message',
                    'comment_author', 'comment_published', 'comment_likes']
    print("ok!!!!!!!!!!!!!!!!!!!!!")
    with open('%s_facebook_comments.csv' % file_id, 'w') as file:
        print("ok!!")
        w = csv.writer(file)
        w.writerow(initial_data)

    print(type(initial_data))
    print(type(initial_data[1]))
    #w.write(initial_data)
                    ...
                    if 'paging' in comments:
                        if 'next' in comments['paging']:
                            comments = json.loads(request_until_succeed(
                                        comments['paging']['next'].encode("utf-8")))
                        else:
                            has_next_page = False
                    else:
                        has_next_page = False
